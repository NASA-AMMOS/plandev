...
/**
 * Record the digest of a model's declared type surface.
 *
 * <p>Deliberately indifferent to where the digest came from. For a live external backend it is the
 * identityHash the backend reported; for a {@code declared} model it is computed from the run transfer
 * file that declared the types. Both answer the same question -- "were these results produced against
 * the model PlanDev has, or a drifted one?" -- so they share one column rather than two that mean the
 * same thing.
 *
 ...
 * re-declaration that carries the same hash must be a genuine no-op -- writing the same value back
 ...
 * first write for a model registered before this column existed does update it (and bumps the
