create view merlin.scheduling_sources as (
  ...
  from (
  	select a.scheduled_directive_id, a.scheduled_plan_id, jsonb_build_object('type', 'activity', 'value', a.referenced_directive_id, 'valid', a.valid) AS sources
  	from merlin.directive_source_is_activity as a
  	union all

    select r.scheduled_directive_id, r.scheduled_plan_id, jsonb_build_object('type', 'resource', 'value', r.referenced_resource_name, 'valid', r.valid)
