...
/**
 * Benchmark test for serialization/deserialization performance.
 *
 * This test creates complex data structures and measures:
 * 1. Domain Object -> SerializedValue (ValueMapper.serializeValue)
 * 2. SerializedValue -> JSON (JsonEncoding.encode)
 * 3. JSON -> SerializedValue (JsonEncoding.decode)
 * 4. SerializedValue -> Domain Object (ValueMapper.deserializeValue)
 *
 * Run with: ./gradlew :serialization-benchmark:test --tests SerializationBenchmarkTest
 */
public class SerializationBenchmarkTest {
  ...
  public void benchmarkSimpleRecordSerialization() {
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; i++) {
      ...
      var json = JsonEncoding.encode(serialized);
      var decoded = JsonEncoding.decode(json);
    ...
    for (int i = 0; i < BENCHMARK_ITERATIONS; i++) {
      JsonEncoding.encode(serialized);
    }
    ...
    System.out.printf("""
        ...
        === Simple Record (Vector3D) Benchmark ===
        Iterations: %d
  ...
  public void benchmarkComplexNestedSerialization() {
    ...
    System.out.printf("""
        ...
        === Complex Nested Record (ComplexActivityState) Benchmark ===
        Iterations: %d
        Serialize (Object -> SerializedValue): %.2f ms (%.2f µs/op)
        Encode (SerializedValue -> JSON):      %.2f ms (%.2f µs/op)
        Decode (JSON -> SerializedValue):      %.2f ms (%.2f µs/op)
  ...
  public void benchmarkListSerialization() {
    ...
    var json = JsonEncoding.encode(mapper.serializeValue(testData));
    System.out.printf("""

        === List Serialization Benchmark (100 TelemetryPoints) ===
        Iterations: %d
        Full Round-trip Time: %.2f ms (%.2f µs/op)
        JSON Size: %d bytes
        ...
        json.toString().length(),
        (json.toString().length() * BENCHMARK_ITERATIONS) / (timeMs * 1000)
    );
  }
  ...
  public void benchmarkMapSerialization() {
    ...
    var mapper = new gov.nasa.jpl.aerie.contrib.serialization.mappers.MapValueMapper<>(new gov.nasa.jpl.aerie.contrib.serialization.mappers.StringValueMapper(), 
    ...
    System.out.printf("""
        ...
        === Map Serialization Benchmark (100 entries) ===
        Iterations: %d
        Full Round-trip Time: %.2f ms (%.2f µs/op)
