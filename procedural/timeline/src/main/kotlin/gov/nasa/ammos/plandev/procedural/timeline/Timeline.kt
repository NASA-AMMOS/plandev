...
interface Timeline<V: IntervalLike<V>, THIS: Timeline<V, THIS>>: Iterable<V> {
   ...
   * [`Intervals<Segment<Boolean>>`][gov.nasa.ammos.plandev.procedural.timeline.collections.Universal] to [Booleans][gov.nasa.ammos.plandev.procedural.timeline.collections.profiles.Booleans]
   * without sorting and coalescing the segments could result in an invalid profile. In cases like that, it is better to
   * use [flattenIntoProfile][gov.nasa.ammos.plandev.procedural.timeline.ops.ParallelOps.flattenIntoProfile] or [reduceIntoProfile][gov.nasa.ammos.plandev.procedural.timeline.ops.ParallelOps.reduceIntoProfile].
   */
   ...
   * because the [gov.nasa.ammos.plandev.procedural.timeline.ops.GeneralOps.unsafeOperate] function accesses this for them.
   *
   * @suppress
   */
