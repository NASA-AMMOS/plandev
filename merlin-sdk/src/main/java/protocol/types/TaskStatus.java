...
public sealed interface TaskStatus<Return> {
  ...
  record Delayed<Return>(gov.nasa.jpl.aerie.merlin.protocol.types.Duration delay, Task<Return> continuation) implements TaskStatus<Return> {}

  record CallingTask<Return>(gov.nasa.jpl.aerie.merlin.protocol.types.InSpan childSpan, TaskFactory<?> child, Task<Return> continuation) implements TaskStatus<Return> {}
