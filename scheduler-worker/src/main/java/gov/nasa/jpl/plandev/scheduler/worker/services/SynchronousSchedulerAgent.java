...
import gov.nasa.jpl.plandev.merlin.driver.MissionModelLoader;
...
import gov.nasa.jpl.plandev.merlin.protocol.model.SchedulerPlugin;
...
import gov.nasa.jpl.plandev.scheduler.SchedulingInterruptedException;
...
import gov.nasa.jpl.plandev.scheduler.model.SchedulingCondition;
...
import gov.nasa.jpl.plandev.scheduler.server.exceptions.ResultsProtocolFailure;
import gov.nasa.jpl.plandev.scheduler.server.exceptions.SpecificationLoadException;
...
import gov.nasa.jpl.plandev.scheduler.server.http.ResponseSerializers;
...
import gov.nasa.jpl.plandev.scheduler.server.models.GoalInvocationRecord;
import gov.nasa.jpl.plandev.scheduler.server.models.GoalSource;
import gov.nasa.jpl.plandev.scheduler.server.models.GoalType;
...
import gov.nasa.jpl.plandev.scheduler.server.models.SchedulingCompilationError;
import gov.nasa.jpl.plandev.scheduler.server.models.SchedulingDSL;
import gov.nasa.jpl.plandev.scheduler.server.models.Specification;
import gov.nasa.jpl.plandev.scheduler.server.remotes.postgres.GoalBuilder;
import gov.nasa.jpl.plandev.scheduler.server.services.MerlinDatabaseService;
import gov.nasa.jpl.plandev.scheduler.server.services.MerlinServiceException;
import gov.nasa.jpl.plandev.scheduler.server.services.ScheduleRequest;
import gov.nasa.jpl.plandev.scheduler.server.services.ScheduleResults;
import gov.nasa.jpl.plandev.scheduler.server.services.SchedulerAgent;
import gov.nasa.jpl.plandev.scheduler.server.services.SpecificationService;
...
import gov.nasa.jpl.plandev.scheduler.simulation.SimulationData;
 ...
 * @param outputMode how the scheduling output should be returned to plandev (eg overwrite or new container)
 */
    ...
    implements SchedulerAgent
{
   ...
   * failure reason to the given output port (eg plandev could not be reached, mission model could not be loaded from jar
    ...
    try(final var cachedEngineStore = new InMemoryCachedEngineStore(sizeCachedEngineStore)) {
      ...
      if(lastGoalSimulateAfter && planMetadataAfterChanges.planRev() != specification.planRevision()) {
        datasetId = storeSimulationResults(
            simulationFacade.simulateWithResults(solutionPlan, planningHorizon.getEndPlanDev()),
  ...
  /**
   * fetch just the current revision number of the target plan from plandev services
   *
   ...
   * @throws ResultsProtocolFailure when the requested plan cannot be found, or plandev could not be reached
   */
   ...
   *     changed, or plandev could not be reached
   */
   ...
   *     version/name in jar does not match, or plandev filesystem could not be mounted
   */
   ...
   * @throws ResultsProtocolFailure when the plan could not be stored to plandev, the target plan revision has
   *     changed, or plandev could not be reached
   */
