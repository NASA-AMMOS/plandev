...
/*package-local*/ final class GetModelAction implements AutoCloseable {
  private static final @Language("SQL") String sql = """
    select m.mission, m.name, m.version, m.owner, m.model_type, m.external_backend, m.external_model_key, m.external_identity_hash, m.external_capabilities::text, encode(f.path, 'escape')
  ...
  public Optional<MissionModelRecord> get(final long modelId) throws SQLException {
    ...
    try (final var results = this.statement.executeQuery()) {
      ...
      final var externalIdentityHash = results.getString(8);
      final var externalCapabilities = results.getString(9);
