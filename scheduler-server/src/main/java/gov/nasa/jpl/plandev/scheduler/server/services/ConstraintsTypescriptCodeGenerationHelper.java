...
import gov.nasa.jpl.plandev.constraints.TypescriptCodeGenerationService;
    ...
    public static Map<String, TypescriptCodeGenerationService.ActivityType> activityTypes(final MerlinDatabaseService.MissionModelTypes missionModelTypes) {
        return missionModelTypes
            ...
            .map(activityType -> Map.entry(
                activityType.name(),
                new gov.nasa.jpl.plandev.constraints.TypescriptCodeGenerationService.ActivityType(
                    activityType
                        .parameters()
                        .entrySet()
                        .stream()
                        .map(entry -> new gov.nasa.jpl.plandev.constraints.TypescriptCodeGenerationService.Parameter(
                            entry.getKey(),
                            entry.getValue()))
                        .toList())))
