...
public final class DumbRecurrenceGoal implements ProcedureMapper<gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal> {
  @Override
  public ValueSchema valueSchema() {
    return AutoValueMappers.gov_nasa_jpl_aerie_e2e_procedural_scheduling_procedures_DumbRecurrenceGoal(
            BasicValueMappers.$int(),
            BasicValueMappers.$int()).getValueSchema();
  }

  @Override
  public InputType<gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal> getInputType(
      ) {
    return new InputMapper();
  }

  public final class InputMapper implements InputType<gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal> {
    ...
    public Map<String, SerializedValue> getArguments(
        final gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal input) {
    ...
    @Override
    public gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal instantiate(
        final Map<String, SerializedValue> arguments) throws InstantiationException {
      final var defaults = new gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal.Defaults();
      Optional<Integer> quantity = Optional.empty();
      Optional<Integer> biteSize = Optional.empty();

      quantity = Optional.ofNullable(defaults.quantity);

      final var instantiationExBuilder = new InstantiationException.Builder("DumbRecurrenceGoal");
      ...
      return new gov.nasa.jpl.aerie.e2e.procedural.scheduling.procedures.DumbRecurrenceGoal(quantity.get(), biteSize.get());
    }
