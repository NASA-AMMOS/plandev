...
/*package-local*/ final class GetAllModelsAction implements AutoCloseable {
  private static final @Language("SQL") String sql = """
    select m.id, m.mission, m.name, m.version, m.owner,
           m.model_type, m.external_identity_hash, m.external_capabilities::text, f.path
    from merlin.mission_model as m
    -- LEFT, not INNER: see GetModelAction. An inner join here silently drops every JAR-less model
    -- from the list of models merlin knows about.
  ...
  public Map<Long, MissionModelRecord> get() throws SQLException {
    try (final var results = this.statement.executeQuery()) {
      ...
      while (results.next()) {
        ...
        final var externalIdentityHash = results.getString(7);
        final var externalCapabilities = results.getString(8);
        final var pathString = results.getString(9);
