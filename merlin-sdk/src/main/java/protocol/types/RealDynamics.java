...
public final class RealDynamics {
  ...
  public Optional<gov.nasa.jpl.aerie.merlin.protocol.types.Duration>
  whenBetween(final double min, final double max, final gov.nasa.jpl.aerie.merlin.protocol.types.Duration atEarliest, final gov.nasa.jpl.aerie.merlin.protocol.types.Duration atLatest) {
    if (this.rate == 0) {
    ...
    final double valueAtEarliest = this.initial + this.rate * atEarliest.in(gov.nasa.jpl.aerie.merlin.protocol.types.Duration.SECONDS);
    final double valueAtLatest = this.initial + this.rate * atLatest.in(gov.nasa.jpl.aerie.merlin.protocol.types.Duration.SECONDS);
    final gov.nasa.jpl.aerie.merlin.protocol.types.Duration entry;
    if (min <= valueAtEarliest && valueAtEarliest <= max) {
      entry = atEarliest;
    } else if (this.rate > 0) {
      if (valueAtLatest < min || max < valueAtEarliest) return Optional.empty();
      entry = gov.nasa.jpl.aerie.merlin.protocol.types.Duration.roundUpward((min - this.initial) / this.rate, gov.nasa.jpl.aerie.merlin.protocol.types.Duration.SECONDS);
    } else /* this.rate < 0 */ {
      if (valueAtEarliest < min || max < valueAtLatest) return Optional.empty();
      entry = gov.nasa.jpl.aerie.merlin.protocol.types.Duration.roundUpward((max - this.initial) / this.rate, gov.nasa.jpl.aerie.merlin.protocol.types.Duration.SECONDS);
    }
  ...
  public Optional<gov.nasa.jpl.aerie.merlin.protocol.types.Duration>
  whenNotBetween(final double min, final double max, final gov.nasa.jpl.aerie.merlin.protocol.types.Duration atEarliest, final gov.nasa.jpl.aerie.merlin.protocol.types.Duration atLatest) {
    if (this.rate == 0) {
    ...
    if (valueAtEarliest < min || max < valueAtEarliest) {
      ...
      entry = gov.nasa.jpl.aerie.merlin.protocol.types.Duration.roundUpward((min - this.initial) / this.rate, Duration.SECONDS);
    }
